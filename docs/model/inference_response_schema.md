...
- `get_low_certainty_handling()` conservative low-certainty messaging
...
- `low_certainty`: Conservative response-handling flag based on confidence/uncertainty. It does not change predictions.
- `low_certainty_reason`: Human-readable reason when `low_certainty` is true.
- `low_certainty_message`: Educational wording shown when the output should be framed as uncertain.
- `low_certainty_rule`: Plain-language rule used to set `low_certainty`.
- `low_certainty_threshold`: Top-confidence threshold for low-certainty handling.
