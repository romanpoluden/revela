...
_CANCER_CLASS_INDICES = {0, 1}  # Melanoma=0, Non-melanoma skin cancer=1


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate cancer-risk EfficientNet-B0 on BCN20000 test split.")
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Stop after this many batches (smoke test only).",
    )
...
def run_inference(model, loader, device, max_batches: int | None = None):
    all_labels: list[int] = []
...
def compute_confusion_matrix_normalized(matrix: list[list[int]]) -> list[list[float]]:
    normalized = []
    for row in matrix:
        row_sum = sum(row)
        normalized.append([v / row_sum if row_sum > 0 else 0.0 for v in row])
    return normalized
...
def compute_classification_metrics(
    ...
    for i, name in enumerate(class_names):
        ...
        fp = sum(confusion_matrix[r][i] for r in range(num_classes) if r != i)
        fn = sum(confusion_matrix[i][c] for c in range(num_classes) if c != i)
        ...
        per_class_rows.append({"class_name": name, "precision": precision, "recall": recall, "f1": f1, "support": support})
...
def compute_cancer_metrics(confusion_matrix: list[list[int]]) -> dict[str, float]:
    cancer_indices = sorted(_CANCER_CLASS_INDICES)
    total_cancer_true = sum(sum(confusion_matrix[i]) for i in cancer_indices)
    cancer_tp = sum(confusion_matrix[i][j] for i in cancer_indices for j in cancer_indices)
    cancer_recall = cancer_tp / total_cancer_true if total_cancer_true > 0 else 0.0
    cancer_fnr = 1.0 - cancer_recall

    mel_row = confusion_matrix[0]
    mel_support = sum(mel_row)
    melanoma_recall = mel_row[0] / mel_support if mel_support > 0 else 0.0

    nmsc_row = confusion_matrix[1]
    nmsc_support = sum(nmsc_row)
    nmsc_recall = nmsc_row[1] / nmsc_support if nmsc_support > 0 else 0.0
...
def save_metrics_json(
    ...
    payload = {
        ...
        "cancer_fnr": metrics["cancer_fnr"],
        "melanoma_recall": metrics["melanoma_recall"],
        "nmsc_recall": metrics["nmsc_recall"],
...
def save_classification_report_csv(output_path: Path, class_metrics: list[dict]) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def save_confusion_matrix_png(
    output_path: Path, confusion_matrix: list[list[int]], class_names: list[str]
) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    draw.text((20, 20), "BCN20000 Cancer-Risk Confusion Matrix", fill="black", font=font)
    draw.text((20, 55), "Rows = true labels, columns = predicted labels", fill="black", font=font)

    for idx, name in enumerate(class_names):
        x = left_margin + idx * cell_size
        y = top_margin + idx * cell_size
        draw.text((x + 10, 85), name[:20], fill="black", font=font)
        draw.text((20, y + cell_size // 2 - 8), name[:30], fill="black", font=font)
    ...
    for row_idx in range(num_classes):
        for col_idx in range(num_classes):
            ...
            draw.text((x0 + (cell_size - text_w) / 2, y0 + (cell_size - text_h) / 2), text, fill="black", font=font)

    image.save(output_path)


def print_metrics(
    metrics: dict, class_metrics: list[dict], class_names: list[str], confusion_matrix: list[list[int]]
) -> None:
    ...
    print(f"  - cancer_recall:     {metrics['cancer_recall']:.4f}  (Melanoma + NMSC predicted as any malignant class)")
    print(f"  - cancer_fnr:        {metrics['cancer_fnr']:.4f}")
    print(f"  - melanoma_recall:   {metrics['melanoma_recall']:.4f}")
    print(f"  - nmsc_recall:       {metrics['nmsc_recall']:.4f}")
    print("")
    ...
    header = "true\\pred".ljust(20) + "".join(name[:18].ljust(20) for name in class_names)
    print(header)
    for name, row in zip(class_names, confusion_matrix):
        print(name[:18].ljust(20) + "".join(str(v).ljust(20) for v in row))
...
def main() -> None:
    ...
    true_labels, predictions = run_inference(model, test_loader, device, max_batches=args.max_batches)
    confusion_matrix = compute_confusion_matrix(true_labels, predictions, num_classes)
    confusion_matrix_normalized = compute_confusion_matrix_normalized(confusion_matrix)
    class_metrics, macro_f1, balanced_accuracy = compute_classification_metrics(confusion_matrix, class_names)
    cancer_metrics = compute_cancer_metrics(confusion_matrix)

    metrics = {
        "num_test_examples": len(true_labels),
        "top1_accuracy": compute_top1_accuracy(true_labels, predictions),
        "macro_f1": macro_f1,
    ...
    metrics_json_path = Path("outputs/metrics/bcn20000_cancer_risk_test_metrics.json")
    report_csv_path = Path("outputs/metrics/bcn20000_cancer_risk_classification_report.csv")
    confusion_png_path = Path("outputs/plots/bcn20000_cancer_risk_confusion_matrix.png")

    save_metrics_json(metrics_json_path, metrics, confusion_matrix, confusion_matrix_normalized, class_metrics, class_names)
